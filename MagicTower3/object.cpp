...
void Scence::FightTread(SMonster &mster)
{
    ...
    while(!e){
        ...
        Tools::printFormattedOutput(80,2,{"e. quit","m. medicen："+to_string(player->Getbag().GetmLifeAmount())});
        Tools::Delaymoment(2000);
...
void Scence::CinTread()
{
    while (1) {
        ...
        if (input == 'e') {
            ...
            player->GetHP()+=player->Getbag().GetmLifeAmount();
            player->Getbag().GetmLifeAmount()=0;
            //todo
            vector<PropPtr>& vbag=player->Getbag().GetVbag();
            vbag.erase(remove_if(vbag.begin(),vbag.end(),[](PropPtr &bag){
                return  bag->Getm_type()==Type::MEDICINE;
            }),vbag.end());
        }else{
            break;
        }
...
void Scence::MoveMonsters(SMonster &mster)
{
    ...
    // 创建战斗线程，使用 std::bind 来绑定成员函数
    std::thread fightThread(std::bind(&Scence::FightTread, this, std::ref(mster)));
    // 创建输入线程
    std::thread cinThread(&Scence::CinTread, this);
    // 等待战斗线程结束
    fightThread.join();
    // 如果输入线程还在运行，则分离它
...
void Bag::Discard(int id)
{
    ...
    //     Vbag.e
    // }
    auto ite= find_if(Vbag.begin(),Vbag.end(),[id](PropPtr& prop){
        return prop->Getmprid()==id;
    });
    if(ite==Vbag.end()){
        Tools::printFormattedOutput(80,1,{"this id is not exit!!!"});
        Tools::Delaymoment(1000);
        return ;
    }
...
void Bag::Unquip(int id)
{
    ...
    auto it=find_if(Vbag.begin(),Vbag.end(),
    [id](const PropPtr& sprop){
      return  sprop->Getmprid()==id&&sprop->Getm_state()==State::LOADING;
    });
    if(it==Vbag.end()){
        Tools::printFormattedOutput(80,1,{"this id is not exit!!!"});
        Tools::Delaymoment(1000);
...
void Menu::Play()
{
    ...
    ISPlay=true;
    // *(mpscence->Getplayer())=mpscence->Getplayer()->GetOldPlayer();
