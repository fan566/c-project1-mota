...
public:
    ...
    void Save(std::fstream& f) const ;
    void Load(std::fstream& f,Shop& shop);
    Player& GetOldPlayer(){return *player;};
...
public: 
    BuildCPlayer(){
        m_player=make_shared<Player>(0,0,"🤓","小明",100,100,10,5,5,0);
    }
...
public:
    ...
    void FightTread(SMonster &mster);
    void CinTread();
private:
...
public:
    Menu(SScence pscence):mpscence(pscence),ISPlay(false){}
    void MenuDisplay();
    void Play();
    void Continue();
    void Archive();
